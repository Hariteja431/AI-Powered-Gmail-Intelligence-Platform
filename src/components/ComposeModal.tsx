          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-gray-200/60 p-1 rounded-lg">
              <button
                ...
                className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${mode === 'prompt' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
              >
                AI Prompt
              </button>
              <button
                onClick={() => setMode('edit')}
                className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${mode === 'edit' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Manual Edit
              </button>
            </div>
            ...
            {mode === 'prompt' ? (
              <div className="space-y-3">
                <textarea
                  ...
                  placeholder="E.g., Write a polite email asking for a follow-up meeting next Tuesday at 10 AM..."
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-all resize-none h-32"
                />
                <button
                  onClick={handleDraft}
                  disabled={isDrafting || !instruction.trim()}
                  className="flex items-center justify-center w-full gap-2 bg-purple-100 text-purple-700 hover:bg-purple-200 px-4 py-2 rounded-lg font-medium transition-colors disabled:opacity-50"
                >
                  {isDrafting ? (
